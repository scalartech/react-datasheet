...
var computeRowVirtualization = function computeRowVirtualization(_ref) {
  ...
  var pinnedRows = Math.min(isNonNegativeInteger(pinnedRowCount) ? pinnedRowCount : 0, Math.max(0, totalRows));
...
var computeColumnVirtualization = function computeColumnVirtualization(_ref2) {
  ...
  var pinnedCols = Math.min(isNonNegativeInteger(pinnedColumnCount) ? pinnedColumnCount : 0, totalCols);
    ...
    value: function render() {
      ...
      if (virtualization) {
        ...
        if (pinnedRowCount != null && !isNonNegativeInteger(pinnedRowCount)) {
          throw new Error('Invalid virtualization: pinnedRowCount must be a finite non-negative integer.');
        }
        if (pinnedColumnCount != null && !isNonNegativeInteger(pinnedColumnCount)) {
          throw new Error('Invalid virtualization: pinnedColumnCount must be a finite non-negative integer.');
        }
      }
      ...
      var pinnedRows = Math.min(isNonNegativeInteger(pinnedRowCount) ? pinnedRowCount : 0, data.length);
      ...
      var renderRowContent = function renderRowContent(row, i) {
        var renderCell = function renderCell(cell, j) {
          ...
          if (isPinnedRow || isPinnedCol) {
            ...
            if (isPinnedRow && typeof rowHeight === 'number' && Number.isFinite(rowHeight)) {
