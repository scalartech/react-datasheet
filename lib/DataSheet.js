...
var computeColumnVirtualization = function computeColumnVirtualization(_ref2) {
  var totalCols = _ref2.totalCols,
    ...
    overscanCount = _ref2.overscanCount;

  var overscan = typeof overscanCount === 'number' ? overscanCount : 5;
  var safeScrollLeft = scrollLeft || 0;
  var start = Math.max(0, Math.floor(safeScrollLeft / columnWidth) - overscan);
  var end = Math.min(
    totalCols - 1,
    Math.floor((safeScrollLeft + viewportWidth) / columnWidth) + overscan,
  );
  var visibleCount = end >= start ? end - start + 1 : 0;
  var leftPad = start * columnWidth;
  var rightPad = Math.max(
    0,
